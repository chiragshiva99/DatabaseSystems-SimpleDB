...
public class HeapPage implements Page {
    ...
    public void deleteTuple(Tuple t) throws DbException {
        ...
        if(rid == null) throw new DbException("Delete Failure: tuple slot already deleted");
        if(!rid.getPageId().equals(pid)) throw new DbException("Delete Failure: tuple not on page");

        int j = rid.getTupleNumber();
        if(!isSlotUsed(j)) throw new DbException("Delete Failure: Empty tuple slot");

        tuples[j] = null;
        markSlotUsed(j,false);
    }
    ...
    public void insertTuple(Tuple t) throws DbException {
        // some code goes here
        // not necessary for lab1
        if(getNumEmptySlots() == 0) throw new DbException("Insert Failure: page full (no empty slots)");
        if(!t.getTupleDesc().equals(td)) throw new DbException("Insert Failure: tupledesc mismatch");
        ...
        throw new DbException("Insert Failure: page full (no empty slots)");
    }
    ...
    public boolean isSlotUsed(int i) {
        ...
        return (byte) (header[byte_idx] << (7 - slot_idx)) < 0;
    }
    ...
    public Iterator<Tuple> iterator() {
        // some code goes here
        ArrayList<Tuple> tuple_iterator = new ArrayList<Tuple>();

        for (int i = 0; i < tuples.length; i++) {

            if (isSlotUsed(i)) {
                tuple_iterator.add(tuples[i]);
            }
        }

        return tuple_iterator.iterator();
    }
