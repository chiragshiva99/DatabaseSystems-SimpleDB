        ...
        while(this.child1.hasNext () || this.tuple != null) {
            ...
            while(child2.hasNext ()) {
                ...
                if (p.filter (tuple, tuple2)) {
                    TupleDesc tupleDesc1 = tuple.getTupleDesc();
                    TupleDesc tupleDesc2 = tuple2.getTupleDesc();
                    TupleDesc newTupleDesc = TupleDesc.merge (tupleDesc1, tupleDesc2);
                    Tuple newTuple = new Tuple(newTupleDesc);
                    newTuple.setRecordId(tuple.getRecordId());

                    int i=0;
                    for (; i<tupleDesc1.numFields(); ++i)
                        newTuple.setField(i, tuple.getField(i));
                    for (int j=0; j<tupleDesc2.numFields(); ++j)
