...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
        ...
        if (pid.pgcateg() == BTreePageId.LEAF){
			...
			Iterator<BTreeEntry> currentpgIterator = currentPage.iterator();
			BTreeEntry entry = null;
			try{
				if(currentpgIterator.hasNext()){
					entry = currentpgIterator.next();
				}
			...
			if (f == null){
				...
				while (currentpgIterator.hasNext() && entry.getKey().compare(Op.LESS_THAN,f)) {
					entry = currentpgIterator.next();
				}
	...
	public BTreeLeafPage splitLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		int j = page.getNumTuples()/2;
		Iterator<Tuple> iterator = page.reverseIterator();
		
		Tuple leftTup = null;
		for (int i = 0; i < j; i++){
			if (iterator.hasNext()){
				Tuple newTup = iterator.next();
				leftTup = newTup;
				page.deleteTuple(newTup);
				newPage.insertTuple(newTup);
			}
		}
		BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), leftTup.getField(keyField));
		newPage.setParentId(parent.getId());
		page.setParentId(parent.getId());
		BTreeEntry newEntry = new BTreeEntry(leftTup.getField(keyField),page.getId(),newPage.getId());
		...
		if (field.compare(Op.GREATER_THAN, leftTup.getField(keyField))){
			return newPage;
		} else{
			return page;
		}		
	...
	public BTreeInternalPage splitInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		// check both pages have equal number of entries
		boolean even = page.getNumEntries()%2==0; 
		BTreeEntry entryUp = null;
		BTreeEntry extEntry = null;
		if (even){
			BTreeEntry currentEntry = iterator.next();
			if (field.compare(Op.GREATER_THAN, currentEntry.getKey())){
				entryUp = currentEntry;
			}
			extEntry = currentEntry;
		}
		if (entryUp == null){
			if (extEntry == null){
				if (iterator.hasNext()){
				...
				page.deleteKeyAndRightChild(extEntry);
				newPage.insertEntry(extEntry);
			}
			if (iterator.hasNext()){
				entryUp = iterator.next();
			}
		} 
		// push entry to parent
		BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), field);
		page.deleteKeyAndRightChild(entryUp);
		entryUp.setLeftChild(page.getId());
		entryUp.setRightChild(newPage.getId());
		parent.insertEntry(entryUp);
		updateParentPointers(tid, dirtypages, newPage);
		if (field.compare(Op.GREATER_THAN, entryUp.getKey())){
			return newPage;
		} else{
			return page;
		}	
	...
	public void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		Field newKey;
		int numMove = (sibling.getNumTuples()-page.getNumTuples())/2;
		Tuple newTuple = null;
		if (isRightSibling){
		...
		for (int i = 0; i < numMove; i++){
			if (iterator.hasNext()){
				newTuple = iterator.next();
				sibling.deleteTuple(newTuple);
				page.insertTuple(newTuple);
			}
		}

		if (isRightSibling){
			newKey = iterator.next().getField(keyField);
		} else{
			newKey = newTuple.getField(keyField);
		}
		entry.setKey(newKey);
	...
	public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		int numMove = (leftSibling.getNumEntries()-page.getNumEntries())/2;
		Iterator<BTreeEntry> iterator = page.iterator();
		BTreeEntry entry = iterator.next();
		parentEntry.setRightChild(entry.getLeftChild());

		iterator = leftSibling.reverseIterator();
		entry = iterator.next();
		parentEntry.setLeftChild(entry.getRightChild());
		...
		leftSibling.deleteKeyAndRightChild(entry);
		entry.setLeftChild(leftSibling.getId());
		entry.setRightChild(page.getId());
	...
	public void stealFromRightInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		// the corresponding parent entry. Be sure to update the parent
		// pointers of all children in the entries that were moved.
		int numMove = (rightSibling.getNumEntries()-page.getNumEntries())/2;
		Iterator<BTreeEntry> iterator = page.reverseIterator();
		BTreeEntry entry = iterator.next();
		parentEntry.setLeftChild(entry.getRightChild());

		iterator = rightSibling.iterator();
		entry = iterator.next();
		parentEntry.setRightChild(entry.getLeftChild());
		...
		entry.setRightChild(rightSibling.getId());
		parent.insertEntry(entry);
		updateParentPointers(tid, dirtypages, page);
	}
	...
	public void mergeLeafPages(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		while (iterator.hasNext()){
			Tuple newTuple = iterator.next();
			rightPage.deleteTuple(newTuple);
			leftPage.insertTuple(newTuple);
		}
		BTreePageId rightPageNextId = rightPage.getRightSiblingId();
		if (rightPageNextId != null){
			BTreeLeafPage rightRightPage = (BTreeLeafPage) getPage(tid, dirtypages, rightPageNextId, Permissions.READ_WRITE);
			rightRightPage.setLeftSiblingId(leftPage.getId());
		}
		leftPage.setRightSiblingId(rightPageNextId);
	...
	public void mergeInternalPages(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		// Delete the entry in the parent corresponding to the two pages that are merging -
		// deleteParentEntry() will be useful here
		Iterator<BTreeEntry> iterator = leftPage.reverseIterator();
		BTreePageId rightPg = iterator.next().getRightChild();
		parentEntry.setLeftChild(rightPg);
		iterator = rightPage.iterator();
		BTreeEntry entry = iterator.next();
		parentEntry.setRightChild(entry.getLeftChild());
		deleteParentEntry(tid, dirtypages, leftPage, parent, parentEntry);
